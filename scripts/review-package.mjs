...
const narrative = {
  ...
  implemented: [
    ...
    ["Religion as a profile field on both clients: asked once at onboarding, always declinable, shown on a profile only when disclosed, editable afterwards, and a free discovery filter. No caste, community, sub-caste, horoscope or denomination -- and none is coming",
      "supabase/migrations/*religion*, apps/web/src/features/auth/screens/ReligionScreen.tsx, apps/mobile/app/onboarding/religion.tsx"],
  ...
  mockedOrIncomplete: [
    ["Phone verification works on the web and is proven in production: a real Indian number was verified on eraya.app through the MSG91 widget with hCaptcha enabled. The app is a separate path — it uses the OTP API, and until 2026-09-23 the build production served never called MSG91 at all; see the next row. With that closed, the app now fails honestly and the remaining blocker is a template, which on the same day turned out to be a larger obstacle than \"pending approval\" suggested: Eraya owns no DLT registration at all. `getWidgetProcess` reports the live widget's MOBILE and RETRY processes with an **empty** `templateId`, so the web's SMS ride MSG91's own DLT entity — which is how a working widget and an empty Templates page are both true, and why there is nothing for the app to borrow. The OTP API requires a template registered to Eraya's own Principal Entity, so the app's phone step is blocked on operator paperwork rather than on a dashboard field. On 2026-09-22 it became **optional on both clients**, and on 2026-09-24 it was **removed from the app entirely for the private Android beta**: onboarding now begins at Name and runs nine questions rather than ten, and the \"Verify phone\" control is gone from You → Verification. No placeholder stands in either place, because a disabled control or a \"coming soon\" is the same promise made worse by being unactionable. Nothing is written in the removed step's place — `onboarding_completed` already sorts above `phone_verified`, so a member who finishes on the app is past that point on both clients without a stage being recorded on their behalf. What \"Phone verified\" means is untouched: still `phone_verified_via = 'msg91'`, still written only by an edge function holding the service role, still shown on the Verification screen and the You tab, including for a member who verified through the website. Both phone screens are left working and reached by nothing, so restoring the step is putting \"phone\" back at the head of `onboardingSteps`, the two navigations in `features/auth/routing.ts` and the action in `app/you/verification.tsx` — not a rebuild. The website is untouched and still verifies through the MSG91 widget. One consequence worth naming: somebody mid-onboarding on the app who opens the website is asked for a number there, which is benign because that flow works, and stops once they finish", "supabase/functions/phone-otp-* (app), phone-widget-* (web, done), apps/mobile/src/features/auth/routing.ts", "removed from the app for the beta"],
    ["**The app's phone verification has never reached MSG91, and what runs in its place mints real verified marks.** Found 2026-09-23 while diagnosing why no SMS arrived on an Android build. `phone-otp-request` and `phone-otp-verify` are version 8, deployed 2026-09-06T05:54:26Z; `OTP_DEV_CODE` was set on the hosted project eight seconds later, at 05:54:34Z, in the same sitting. The `isLocalStack()` guard that makes the development stub unreachable on a deployed project arrived ten days later in `519cb28` (2026-09-16, \"shut the development hole\") and **those two functions have never been redeployed**, so production runs `developmentCode()` with no gate: `sendOtp` returns success without calling the provider and `verifyOtp` accepts that one fixed code. The source says otherwise and is correct; the deployment is what members meet. Three things make this worse than a stalled feature. `complete_phone_otp` writes `phone_verified_via = 'msg91'`, which is exactly what `phone_is_verified()` requires, so a static six-digit secret mints the trust mark this product withheld for months on principle — bounded by the five-attempt limit and the daily caps, never rotated, and shared with whoever has seen the dashboard. The audit trail reads as success: `phone_otp_sent` rows were written for both attempts on 2026-09-23 at 13:29Z and 13:30Z. And the screen tells the member a code was sent, in a sentence hardcoded in English that bypasses the locale files the app had just been given. **Closed the same day, at 14:14:33Z**, by deploying the functions that were already written: both are now v9, and neither `index.ts` changed by a character — the entire delta was the nine-line guard in `_shared/msg91.ts`. The three `phone-widget-*` functions were left untouched, so the web path is bit-for-bit what it was. The close was then measured rather than assumed, which matters more than the fix: a throwaway account called `phone-otp-request` with a well-formed number, and production answered `unavailable` and recorded `not_configured` — a status reachable only when `developmentCode()` returns null — where the identical call two hours earlier had answered `sent`. The account was deleted afterwards, so its rows cascaded away and no member's allowance was spent; MSG91 was never contacted. One limit is worth stating rather than glossing: `phone-otp-verify`'s development branch cannot be exercised end to end while no template exists, because opening a verifiable request row requires a genuine send. What is established is that the gated `msg91.ts` is the deployed one, uploaded with both functions in a single command, and that `verifyOtp` reads the same `developmentCode()`. `OTP_DEV_CODE` remains set on the project and is now inert", "supabase/functions/_shared/msg91.ts, supabase/functions/phone-otp-request, phone-otp-verify", "shipped"],
    ["Account deletion from the web failed in production and the account survived. The Worker `eraya-web` carried no secrets and no environment bindings at all, so `getServiceRoleKey()` threw, the server action rejected before `deleteUser` was reached, and the member was shown a generic try-again message. Configuration, not code — no code change fixed it. `SUPABASE_SECRET_KEY` was set on 2026-09-22 and the live version now lists it, so the cause is gone, and on 2026-09-23 an account was deleted through eraya.app and the path confirmed end to end. The database side was never in doubt (every migration applied; cascades verified in production against throwaway accounts carrying interests, a connection, messages, blocks, reports, moderation rows and a subscription)", "apps/web/src/features/account/actions.ts, apps/web/src/lib/supabase/env.ts, apps/web/wrangler.jsonc", "shipped"],
    ["Web phone verification reported every failure as an outage. `describeAuthError` renders by error *kind*, and every phone refusal except a cooldown carries the kind `generic`, so a number already registered to another account, a daily cap and a dead widget all read as \"Something went wrong on our side. Please try again.\" A cooldown was worse than generic: it mapped to `rate_limited`, whose copy is about email links and spam folders. Found 2026-09-22 when a second account could not verify a number the first had taken. Shipped 2026-09-22 in `924719c`: the screens render the sentence written for the status, and `phone-widget-begin` (v2) returns `number_taken` rather than masking it, so a number already in use now says so outright. Both halves are live and production was confirmed serving the new copy by fetching its own JS bundle, and the flow was walked by hand on 2026-09-23 when the whole verification path was proved end to end", "apps/web/src/features/auth/describeAuthError.ts, phone-verification.ts, supabase/functions/phone-widget-begin", "shipped"],
    ...
    ["**Nobody could verify a phone number on the web**, for about twenty minutes on 2026-09-22. The captcha was not visible on the phone screen, so the first send could not be completed and sign-up stopped there — a worse fault than the resend bug the change was fixing. The cause was placement, not lifecycle: `AuthLayout` opens with `min-h-dvh` and fills the viewport by design, so the container appended after it in `(auth)/layout.tsx` began exactly one screen below the top of the page, outside the centred column. It was present, valid and initialised before MSG91 ran — production HTML confirmed exactly one such element — and simply off-screen. The container had been doing two jobs and only one survived being hoisted: a target MSG91 can draw into, and a place in the reading order where a person meets the challenge before pressing Continue. Fixed in `4a48aab` (PR #64 → develop, #65 → main): the element is `sticky bottom-0`, centred to the form's column width, and paints nothing while it is empty. It is deployed, and unlike the two fixes before it, it was proved rather than inferred — a stand-in was injected into the live container on eraya.app and its `getBoundingClientRect()` measured against the viewport, because nothing this repository can run sees an element one screen too low. The bug the sequence began with was never in any of this. It was found on 2026-09-23 and is the row below: `retryOtp` was being passed a null channel, which this widget refuses. Three deploys were spent on the captcha because the failure arrived as a generic outage sentence and the captcha was the last thing that had been touched — which is the part worth keeping, not the fix", "apps/web/src/app/(auth)/layout.tsx, features/auth/components/AuthLayout.tsx", "shipped"],
    ["**Resend has never worked on the web, and the cause was one argument.** `window.retryOtp(null, …)` is correct for a widget of type \"1\", where the channel is ignored. Eraya's is type \"2\" — Custom — and there the channel is mandatory: MSG91's `validateInputData` builds an `Error` when it is missing, and `retryOtp` is the only one of the three exposed methods that calls that function *without* handing it the failure callback. So the widget threw synchronously instead of reporting through the callback it had been given. The throw landed inside the promise executor, rejected instantly, and reached the member as \"We could not send your code just now\" — the same sentence an outage produces, carrying none of MSG91's own wording. That is why four separate diagnoses missed it: three aimed at the captcha, one at the rate limits, each sound about the code it changed and none of them about this. Fixed 2026-09-23 in `0eb1e47` (PR #73 → develop, #74 → main) by naming the channel, read from `getWidgetData().globalDefaultChannel` rather than hardcoded because the value belongs to the dashboard. Confirmed against the live widget: `widgetType` is `2`, `globalDefaultChannel` is `11`, and the RETRY process is SMS. Deployed, verified in the served bundle -- which no longer contains a bare `retryOtp(null)` anywhere -- and then **walked by hand on eraya.app: a second SMS arrived and the newest code verified.** That last sentence is the one that counts. Three earlier fixes to this button passed every check available to this repository and were wrong, and the only thing that separated this one from them was somebody watching a message land on a handset", "apps/web/src/features/auth/msg91-widget.ts, scripts/phone-limits-probe.mjs", "shipped"],
    ["The captcha was in the wrong place and said nothing about when it mattered. It sat stuck to the bottom of the viewport on every screen in the auth group -- the name, the birthday, the photograph -- because the element cannot be unmounted between screens and that was the only arrangement found that survived the journey. Continue was pressable before the challenge was answered, so the first press failed, and the box that would have fixed it was somewhere else on the page. Corrected 2026-09-23 in `6e62cc8` (PR #76 -> develop, #77 -> main): the container now has a parked position off-screen in the `(auth)` layout and `CaptchaSlot` borrows it while a screen wants it, so there is still one element and one initialisation and it is seen only on the two screens that verify a number; on the phone screen it sits immediately above Continue in the form's own column; and Continue is disabled until MSG91's own `isCaptchaVerified()` says the challenge is answered. The gate reads the widget's access token and never a checkbox, and when no challenge can be found at all it leaves the button alone, because disabling Continue forever for a widget with captcha validation switched off is a worse fault than showing it early -- the server refuses an unsatisfied send either way. Moving the node was the part this repository had already guessed wrong about three times, so it was established by experiment before it was written: a page was served, the real widget initialised into one container, and the container reparented -- the challenge survived, re-rendered and stayed interactive. Verified in a browser before it shipped -- one challenge rather than two, above Continue, Continue disabled while unanswered, the container returned to its parked position intact on navigating away -- and **walked on eraya.app afterwards**, where a member solved the challenge, received a code and verified. One thing remains unverified and is worth keeping written down rather than assumed: the render on a very narrow phone. hCaptcha's box is a fixed 302px and the column on a 320px device is 280px, so the slot carries `overflow-x-auto` as a guard, which is reasoning rather than a measurement -- no harness available here would drive the viewport below desktop width", "apps/web/src/features/auth/components/CaptchaSlot.tsx, useCaptchaGate.ts, app/(auth)/layout.tsx", "shipped"],
    ["The code screen asked for a captcha nobody needed, offered a button that did nothing useful, and had no way out. A challenge sat under the six boxes even though a resend sends no captcha token at all -- MSG91's `getOtpResendAction` builds its request without one -- so it was furniture on the one screen where a person is already waiting. Continue was pressable with an empty field, which spends a round trip to be told what the screen already knew and answers somebody who typed nothing with an error about what they typed. And the step was optional everywhere except here: the phone screen has always offered a way past it, but the person who wants one has usually already given a number and is waiting for a message that is not coming. Fixed 2026-09-23 in `a0647a1` (PR #79 -> develop, #80 -> main), one file: the captcha is gone from this screen and stays parked, Continue waits for six digits, and Skip for now sits below it. Skipping reuses `recordPhoneStepComplete`, so it writes the stage and nothing else -- proved rather than asserted, by checking afterwards that the profile carried no number, no timestamp and no status, and that the member had zero phone events and zero OTP request rows. Somebody who came from Account -> Verification goes back there with nothing written at all. Verified on eraya.app itself rather than only locally, by reaching the screen through seeded session storage so that no message was sent: no challenge on screen, Continue disabled at nought digits and enabled at six, resend and Change phone number both still present", "apps/web/src/features/auth/screens/OTPScreen.tsx", "shipped"],
    ["Web phone verification refused with \"That is several codes in a short time\" and the refusal outlived the day it was earned. Diagnosed and then fixed on 2026-09-23; the accounting underneath it is now truthful and `npm run phone:probe` holds it there with 39 checks. `begin_phone_otp` returns `user_daily_cap`, which `phone-widget-begin` passes through and the client renders; the audit trail records the same reason on every attempt, so nothing is being masked and MSG91 is never reached — which is why the provider's IP list shows every address Normal and why hCaptcha succeeding proves nothing, the captcha belonging to a widget that is opened after this gate. The rule is a rolling twenty-four hours on `requested_at` against a cap of five, not a calendar day, so five requests made across one morning still refuse the next morning until each rolls out individually. Two genuine defects sat underneath a correct refusal — every limit counting `sent_at` was dead on this client, and the limit that worked counted widget openings rather than codes — and both are repaired; see section N. Nothing was raised to make testing easier and no production row was edited: the historical rows stay as the attempts they certainly were rather than being backfilled with sends nobody observed, and they leave the rolling window on their own", "supabase/migrations/*truthful_otp_send_accounting*, supabase/functions/phone-widget-sent, apps/web/src/features/auth/phone-verification.ts", "shipped, one manual test outstanding"],
    ["**A member who finished signing up met Cloudflare's Error 1102 instead of the product**, at 14:33:51Z on 2026-09-23, six seconds after their stage was written. Not a loop and not the destination page: the same render was reproduced against production nine times — with a throwaway account brought to the incident account's exact shape, signed in through the site's own `/auth/confirm` so the cookies were a browser's — and `/home`, `/account`, `/discovery` and `/connections` all answered 200. What `wrangler tail` shows on the live Worker is the answer. A cold isolate spends **422–659 ms of CPU** before it serves anything; a warm one spends **48–99 ms**. The decisive measurement is `/login` at 659 ms — the cheapest signed-out page in the product costs *more* on a cold isolate than `/home` does, so the cost is the Worker booting rather than any page rendering. The spread is the fault: a distribution whose tail crosses a limit fires intermittently and cannot be provoked on demand, which is exactly what was observed. Completing onboarding is not a cause, only the most reliable way to arrive at a signed-in route on an isolate that has not served one. The largest contribution this repository owns is the locale bundle: `packages/i18n` imports all six dictionaries statically, and they compile to a single **507 KB** chunk that is in the server chunks *and* shipped to the browser, so every cold isolate parses six languages to render one. **SUPERSEDED 2026-09-24 evening -- it is CPU, and the plan is why.** `wrangler tail` reports these invocations as `outcome: exceededCpu` with the exception `Worker exceeded CPU time limit.`, and the kills sit at exactly 10.00 ms. A CPU kill sits *at* the ceiling, and 10 ms is the Workers **free plan** budget -- the plan `wrangler.jsonc` records choosing. The 30 seconds reasoned from below is the *paid* Standard limit, and GraphQL analytics has no `outcome` dimension, which is why it reported `exceededResources` and the memory reading survived a whole day. Startup is not implicated either: the deploy prints `Worker Startup Time: 18-25 ms`. This retro-explains everything -- the locale bundle and the 12-to-1 Supabase client change both cut *per-request* CPU and both helped; removing `@vercel/og` cut *startup* work and changed nothing. A fourth fix (skipping `getUser()` when no `sb-` cookie is present) cut median CPU 34 ms to 23 ms and p90 56 ms to 31 ms, and the signed-in walk now draws 4 `exceededResources` against 87 successes rather than 14. It is not zero and will not reach zero: a dynamic Next.js render costs ~23 ms median against a 10 ms budget, so the remaining options are the paid plan, making public routes static, or leaving Workers. The reasoning preserved below is kept deliberately -- a worked example of a plausible number being adopted as a cause. The original reasoning, now known wrong, followed: it was read as memory rather than CPU from Cloudflare's aggregate analytics. Workers Logs refused the request — wrangler's OAuth token carries no observability scope, so the per-event line for Ray `a3fa4451d979008a` was never read — but the GraphQL API answered it by arithmetic. The Worker runs on the `standard` usage model, whose CPU ceiling is 30 seconds; the five killed invocations had burned 12.8 ms at the median and 65.6 ms at p99, while successful invocations on the same Worker routinely burn 700–811 ms without being touched. Nothing exceeds a thirty-second budget in sixty-six milliseconds, and the only other per-invocation ceiling on Workers is memory: 128 MB, shared by every request an isolate is serving. The corroboration is the other event in the window: on 2026-09-22 at Milan, 174 requests died as `exceededResources` at a flat 10.00 ms of CPU each. No CPU-limit story produces that; an isolate already over its memory ceiling, killing everything assigned to it, produces exactly that. The fix was deployed on 2026-09-24 at 06:19:32Z and is live — Worker version `abf47c37`, carried forward into `37501775` at 07:50:15Z — and **it did not close this**. Walked on production the same morning: all six languages render on every public and onboarding screen, and `/home`, the post-onboarding destination, answers 200 in all six. But a sequential walk of the signed-in routes at roughly one request a second drew **14 fresh `exceededResources` in a single five-minute bucket at Marseille**, on the deployed fix, at 10.0 ms of CPU against 279 ms for healthy requests on the same Worker — the same memory signature as the original. Repeating the walk three seconds apart returned 9/9 at 200 and produced no errors at all. So the honest position is narrower than \"fixed\": the largest contributor Eraya owns has gone, normal single-member pacing is healthy, and the ceiling is still reachable by sustained traffic on signed-in routes. Production was running the 07:52:29Z build when the member met this and the signed-in routes had not changed since 2026-09-22, so no deployment caused the original", "apps/web/wrangler.jsonc, packages/i18n/src/index.ts, apps/web/src/features/i18n/server.ts", "deployed; still reachable under load"],
  ...
  trustNotes: [
    "No trust mark is shown for anything Eraya has not actually checked. 'Phone verified' was withheld from member profiles for as long as verification was mocked, and returned on 2026-09-22 on a narrower footing than the one it left on: `member_card.phone_verified` is built by `phone_is_verified()`, which requires `phone_verified_via = 'msg91'`, so the accounts the stand-in marked — the demo members among them — do not qualify and cannot start qualifying by somebody widening a query. Absence says nothing: verification is optional, so no profile carries 'phone unverified' and no empty slot is left where a mark would be. Neither mark says 'verified profile' or 'verified member' — Eraya has checked a mailbox and sometimes a handset, not a person. The asymmetry between clients is real and accepted: the app cannot verify anybody until its SMS template is approved, so a mark is available to web members and not yet to app members. What it means is identical on both — an SMS was answered — which is the property that matters; the alternative, withholding a true mark from everybody until the slower client catches up, withholds information a reader is entitled to in order to keep a symmetry nobody asked for. **That claim had a live exception for seventeen days, found and closed on 2026-09-23, and it belongs in the note rather than in a footnote:** the app path was served by a stale build whose development stub accepted one fixed code and wrote `phone_verified_via = 'msg91'` for a number nobody checked. The rule was sound in the source and the query is as narrow as it says; what failed is that a deployment can lag a repository by ten days without anything failing, so a mark this product spent months refusing to show could be earned by typing a secret. Closed at 14:14:33Z by redeploying, and confirmed by asking production what it does — section B. No affected row exists: every `phone_verified_via` value in the database is `'mock'`, from the seeded demo members. That is worth stating precisely rather than claiming a clean bill, because it is true by accident — the throwaway accounts that could have held a stub-written mark were deleted for other reasons, not refused at the time.",
    "Copy is constrained to claims defensible on launch day: no member counts, no testimonials, no '100% verified'.",
  ],

  journeys: [
    ["New user registration", "Email address, then a six-digit code, then onboarding. On the **website** a phone step follows and may be verified or declined; the **Android app does not ask for a phone at all** in the private beta, because Eraya holds no DLT registration and the app cannot send an SMS. The step is absent from `nextRouteFor`, not merely skippable. No password exists anywhere in the product.", "Nothing verifies identity beyond control of the mailbox. Email stays compulsory because it *is* the account: it is the sign-in method, and an unreachable mailbox is an unrecoverable account. A phone is an attribute of an account that already exists, so refusing it costs nobody anything. **The last step of this journey failed in production on 2026-09-23**: a member who finished onboarding was served Cloudflare's Error 1102 on the way into the product. The cause is an isolate exceeding its memory ceiling rather than anything in the flow, and it is section B's own row — but it is worth reading here too, because the place it lands is the worst place in the product to lose somebody. They have answered every question and have nothing to show for it, and the screen they get carries Cloudflare's branding rather than Eraya's, so it reads as the product having collapsed at the exact moment they committed to it."],
    ...
    ["Phone verification", "**Optional since 2026-09-22, on both clients.** The step is offered first, and a member may verify, decline and carry on, or come back to it from Account → Verification at any time. Declining writes `onboarding_stage = 'phone_verified'` and nothing else — no number, no timestamp, no status — so a member who skipped is indistinguishable from one who was never asked, and causes zero MSG91 sends. It is offered on both screens of the step, the number and the code, because somebody waiting on a message that never came is the person most likely to want it. Verifying late uses the same screen, the same widget, the same captcha and the same `begin_phone_otp` limits as verifying at signup; there is no second path. Web: MSG91's OTP widget runs in the browser and returns an access token, which `phone-widget-verify` exchanges with MSG91 server-side; the number comes from MSG91's answer and must match the request this account opened. App: the OTP API through `phone-otp-request` / `phone-otp-verify`, because MSG91's widget is a browser SDK and its native SDKs need a config plugin that does not exist.", "**Web: verified in production** on eraya.app with a real Indian number, MSG91 CAPTCHA Validation on and hCaptcha as the provider. **App: never run end to end.** Until 2026-09-23 the deployed functions did not reach MSG91 at all (section B); with that corrected they fail honestly, and the remaining obstacle is that Eraya holds no DLT registration of its own, the web's SMS going out under MSG91's. It is at least no longer a dead end for the member: an app member who cannot get a code declines the step and finishes. Nothing was faked to achieve that — the app still asks MSG91 for a real code and still fails truthfully when MSG91 will not send one. No code is ever stored. The fixed development code is refused unless `SUPABASE_URL` is a local stack — true of the source since 2026-09-16 and, since 14:14:33Z on 2026-09-23, true of the deployment as well. It was not before: the app path spent seventeen days on a build predating that guard, faking every send and accepting one code. Section B. The code screen names the number it sent to, masked to the last two digits, and offers a resend on a sixty-second countdown that is a label on `begin_phone_otp`'s cooldown rather than a limit of its own — and as of 2026-09-23 that is true on the web as well, the cooldown having been unreachable there until the widget path began reporting its sends. The daily allowance is five provider-confirmed codes per account per rolling day, and opening the widget no longer spends one. Section N. The captcha is one element owned by the `(auth)` layout and borrowed by the two screens that verify a number: parked off-screen the rest of the time, so it is never drawn twice and never appears on a screen with no use for it, and shown immediately above Continue on the phone screen, which is disabled until MSG91's own token says the challenge is answered. Getting there cost four deploys and three wrong diagnoses, all of them aimed at the captcha; the resend failure was `retryOtp` being passed a null channel, and the captcha is not involved in a resend at all -- MSG91 sends no captcha token with one. **The whole path was walked on eraya.app on 2026-09-23**: first SMS, sixty-second wait, resend, second SMS, newest code, verified. The code screen carries no challenge of its own, because a resend needs none; it holds Continue until six digits are present, and offers the same way past the step that the phone screen does -- which is where somebody who never received a message actually is when they want it."],
    ["Religion", "Asked once, during onboarding, on both clients. Eight options including \"Prefer not to say\", nothing preselected, and an answer required before continuing -- where declining is one of the answers. Changeable afterwards from Edit profile in the app and from the account screen on the web, including changing it back to \"prefer not to say\", which withdraws it everywhere at once.", "Stored as an enum on `profiles`, null for every account that predates the question -- nothing was backfilled and nothing is ever inferred from a name, a city, a language or anything else. `disclosed_religion()` collapses \"prefer not to say\" and \"never answered\" into the same null before the value leaves the database, so no client receives the refusal and none can render it. The discovery filter is free, offers only disclosed values, and matches on the disclosed value -- so asking for \"prefer not to say\" cannot be used to list the members who declined. Proved by `npm run religion:probe`."],
    ...
    ["Messaging", "Only between connected members.", "No moderation, no reporting inside a thread. An unread conversation is marked by `mark_conversation_read`, written per person per connection, and never disclosed to the other party -- there are no read receipts."],
    ["Noticing something happened", "The navigation carries two counts: connections made since the member last opened their Connections list, and conversations containing something they have not read. Both are derived at read time by `activity_summary()` from the rows themselves; there is no stored counter to drift. Seen-ness is a watermark on the member's own profile row and a per-connection read marker -- both in the database, so the answer survives sign-out and is the same on a second device.", "Conversations, never messages: eight messages from one person is a 1. The sender never counts their own. A blocked or deleted member's conversation stops counting because the whole thing is built on `my_conversations()`, which already drops it. In-app only -- nothing is pushed, mailed or texted, and there is no notification feed."],
  ...
  observations: [
    ...
    ["trust", "Web phone verification is proven in production, with hCaptcha in front of MSG91's send. The app cannot verify anybody until it has a DLT-approved SMS template of its own — and for seventeen days it did the other thing instead, on a stale deploy that never called MSG91 and accepted one fixed code, until that was closed on 2026-09-23 (section B). That used to mean somebody signing up on the app reached a step that could not complete and stopped there, which is the fault that made the step optional on 2026-09-22 rather than the other way round: a compulsory step that depends on a third party's approval is an onboarding that the third party can close. It is now a question that can be answered either way on both clients, and the app's inability to send is a refusal the member can walk past instead of a wall. Making the step optional is also what let the stub sit unnoticed for seventeen days: once nobody is forced through a step, nobody finds out that it is lying, and the first person to press it deliberately was the founder testing an Android build on 2026-09-23.", "supabase/functions/phone-otp-* (app)"],
    ["launch blocker", "**A deployment can lag its repository indefinitely and nothing in this project can see it.** The phone OTP functions on production are ten days behind `develop`; the guard written to close a development hole exists only in git, and the hole it closes is open on the live project (section B). Every check this repository runs — types, lints, builds, probes, `npm run security:probe` — reads the working tree, and the working tree is correct. `supabase secrets list` compounded it by answering the question that *seemed* to matter: the MSG91 key is present and current, so the configuration looked complete while the code consuming it was a build from before the guard existed. What actually settled it was `supabase functions list`, whose `updated_at` timestamps dated the running build, checked against `git log` on the file that changed — a thirty-second check nobody had reason to run, because nothing suggests a deployed function and a committed function can disagree. Worth a reviewer's attention as a shape: migrations reach production by an explicit push and are therefore watched, edge functions reach it by a different explicit push and are not, and the two drift independently of each other and of the client deploys. Neither the web nor the app can tell which build answered them. Until there is a deploy step that ships functions with the code, the honest statement about any edge function is that the repository describes intent and the project describes behaviour. The instance was closed on the day it was found, but the shape was not: the two functions are current again and nothing prevents the next ten-day gap, because no check in this repository reads a deployed version. The cheap version of that check is `supabase functions list`, whose `updated_at` and version number could be compared against the commit touching each function's sources — it needs no production data beyond what the CLI already prints, which is the bar the probes are held to.", "supabase/functions/**, scripts/security-probe.mjs"],
    ["launch blocker", "**Eraya holds no DLT registration, and the working web SMS conceal it.** Established 2026-09-23 by reading the live widget: `getWidgetProcess` returns the MOBILE and RETRY processes with an empty `templateId`, so every code the website has delivered went out under *MSG91's* entity rather than Eraya's. That resolves what had looked like a contradiction — a Templates page with nothing on it and a widget that demonstrably texts Indian handsets — and it moves the app's blocker from a dashboard field to operator paperwork: MSG91's OTP API takes a `template_id` registered to the sender's own Principal Entity, and there is none to copy from the widget. The consequence for the plan is worth being blunt about. Phone verification on the app is not a day of work away; it is a Principal Entity registration against a PAN and GST, then a header, then a content template, each with its own approval, and none of it is code. Everything on Eraya's side of it is finished and now provably honest. Two decisions follow, and both are commercial rather than technical: whether to start that registration, or whether the app should stop pretending to have its own path and reach the same widget the web uses — which MSG91 publishes as a browser SDK, and which `apps/mobile/src/features/onboarding/phone.ts` has already argued against on the grounds that a WebView token bridge cannot be authenticated.", "supabase/functions/_shared/msg91.ts, apps/mobile/src/features/onboarding/phone.ts, scripts/phone-limits-probe.mjs"],
    ["security", "**A short secret listed by the Supabase CLI is not a secret.** `supabase secrets list` prints, for every function secret, what is presented as an opaque digest — and it is a plain unsalted SHA-256 of the value, which was confirmed by hashing a known one (`SUPABASE_URL`) and matching it exactly. Anything with a small keyspace is therefore recoverable by anybody who can run that command: `OTP_DEV_CODE` is six digits, so its entire space is a million hashes and falls in under a second. That is a general property rather than an Eraya bug, and it is worth a reviewer's attention because the instinct it defeats is a reasonable one — treating a digest as a safe thing to show. The rule it argues for: a secret whose value is drawn from a small set should not be a secret at all. Where one is unavoidable, it should be long enough that the digest protects it, and it should not outlive the thing it was set for. `OTP_DEV_CODE` was set on 2026-09-06 for a local stack and is still set today.", "supabase/functions/_shared/msg91.ts"],
    ["trust", "The stub is worse than a mock precisely because it was built to be better than one. It was written to replace \"any six digits will do\" with something defensible — a server-held secret, every SQL rule still enforced, every call logged as `provider=stub` so a deployment running on it would be loud — and the reasoning is sound and is still in the file. What it could not survive was ordinary sequence: the secret was set on 2026-09-06 to make a local stack testable, the guard that would have made the secret harmless on a hosted project was written on 2026-09-16, and nothing carried the second fact to the place the first one was already living. The general shape is worth more than the incident. A safety mechanism added after the thing it makes safe is only as good as the deploy that carries it, and a secret set once stays set forever while the code reading it changes underneath — so the durable control is not to ship the bypass at all, or to key it to something the environment cannot be given by accident, rather than to gate it and trust a later release to arrive.", "supabase/functions/_shared/msg91.ts"],
    ...
    ["privacy", "Religion is asked, and three decisions keep that from becoming sorting. It is always declinable, and declining is stored as a choice rather than as an absence -- which is what lets the product tell somebody who said no from somebody who was never asked, and leave the second group alone. It is never inferred: not from a name, which in this country is the same inference used to discriminate, and not from a city or a language either. And the refusal never leaves the database -- `disclosed_religion()` turns it into null at the boundary, because a client that receives \"prefer not to say\" can render it by accident, and a profile reading \"Religion: Prefer not to say\" announces the question and points at the person who declined. What is deliberately absent is the rest of the biodata form: no caste, community, sub-caste, gotra, horoscope or denomination, and no plan for any.", "supabase/migrations/*religion*"],
    ...
    ["product", "The unread badge counts conversations rather than messages, and that is a product decision rather than an implementation convenience. Eight messages from one person is one person waiting, which is a fact somebody can act on; ten is a number that grows while they are asleep and reads as a debt. The mobile tab had been a dot rather than a number for exactly that reason -- the note arguing for it is still in `_layout.tsx` -- and the number was accepted only once it could be made to count people rather than messages. The colour is the brand terracotta, not red: red is this system's destructive-action and error colour, and a first message from somebody who chose you back should not arrive looking like a warning.", "apps/mobile/app/(tabs)/_layout.tsx, apps/web/src/features/app-shell/NavBadge.tsx"],
    ["technical debt", "The web had never called `mark_conversation_read`. The function has existed since August and the app has always called it; the website simply did not, so a conversation opened on the web stayed unread in the database forever. Nothing surfaced it because nothing on the web rendered unread state -- the bug was invisible precisely until the feature that depended on it was built. Worth a reviewer's attention as a shape rather than an incident: a server-side capability with only one of two clients wired to it has no failing signal, and the second client's absence looks identical to it working.", "apps/web/src/features/members/actions.ts"],
    ...
    ["safety", "**Fixed 2026-09-23.** The web now reports the provider's answer back through `phone-widget-sent`, `confirm_phone_otp_send` writes `sent_at` against the caller's own open reservation, and the three limits below are live on both clients for the first time. What is being counted is stated exactly rather than implied: MSG91's widget invoking its success callback in the browser — the provider accepting the message, not a handset receiving it, because nothing in this architecture can see delivery. The paragraph that follows is what was wrong, kept because the shape of it is the lesson. One missing write disabled three of the five limits on the web, and the first version of this entry understated it. `phone-widget-begin` leaves every row `requested` with `sent_at` null — truthfully, because the browser widget sends the message and the server never learns that it did — and only the app's `phone-otp-request` ever calls `record_phone_otp_send`. Three of `begin_phone_otp`'s checks read `sent_at`: the sixty-second cooldown (`max(sent_at)`), the daily spend cap (`sent_at` within a day, 400) and the purchased-capacity ceiling (`sent_at` not null, 5,000). On the web all three read as though nothing had ever been sent, permanently. The cooldown never fires, so the countdown drawn on the code screen is the only thing spacing out resends; more seriously, the two limits that exist to bound money do not function on that client at all, and `scripts/auth-capacity.mjs` reports off the same counters, so it will read 0% used until the allocation is gone. What still held throughout was the per-user and per-number daily cap, which counted `requested_at` — abuse was bounded at five per account per rolling day, not unbounded, which is why this was a hole in the spending guard rather than an open tap. The general shape is worth more than the incident: a limit reading a column that only one of two clients writes has no failing signal, because a cap that never triggers and a cap that is never reached look identical from outside. Proved now by `npm run phone:probe`, which moves time rather than waiting for it and never contacts MSG91.", "supabase/migrations/*truthful_otp_send_accounting*, supabase/functions/phone-widget-sent, scripts/phone-limits-probe.mjs"],
    ["technical debt", "**Fixed 2026-09-23.** A reservation and a send are now two different things: `begin_phone_otp` opens the first and charges nothing for it, and only a provider-confirmed send moves `sent_at` and spends one of the five. A send MSG91 refused is recorded as `send_failed`, costs no allowance and imposes no cooldown; an abandoned reservation is retired the moment a later send supersedes it, which the probe caught before production did — confirming twice used to count two sends for one message, on a stale row, which is an ordinary thing for a browser on a bad connection to cause. Counting sends rather than askings hands a modified client a way to stay quiet about them, so a separate attempt ceiling (`otp_per_user_daily_attempt_cap`, ten) counts reservations and needs nobody's cooperation; it is deliberately looser than the send cap because its job is to bound a liar, not to ration a member whose captcha failed twice. The cap stays on sends and the ceiling stays on attempts precisely so neither has to do the other's job. What follows is what was wrong. The one web limit that worked counted the wrong event, and it was the harshest. `begin_phone_otp` inserts the request row before the browser opens the widget, and nothing ever closes it: a send MSG91 refuses, a captcha abandoned, a tab shut, a widget that throws — each leaves a row that counts against `otp_per_user_daily_cap` for a full twenty-four hours, indistinguishable from a code that arrived. A resend is charged a whole second row rather than incrementing the `resends` column, which is consequently vestigial on this path. Five codes a day is therefore nearer two real attempts, and the member who spends them on Eraya's own failures is locked out for a day by them. Worth separating from the rule itself, which is working as designed: the window is a rolling twenty-four hours on `requested_at`, not a calendar day, so a lockout earned in the morning is still in force the following morning and reads to everyone — including whoever is testing — as a fault rather than a limit. Refusals themselves cost nothing and are not recorded as requests, so retrying could not deepen the hole.", "supabase/migrations/*supersede_open_otp_reservations*, supabase/functions/phone-widget-sent, apps/web/src/features/auth/phone-verification.ts"],
    ["technical debt", "Two faults in one widget, both invisible to every check this repository can run, and both found only by reading the provider's source. MSG91 gates its `captchaVerified` callback on `config.exposedMethods` while everything else in the same file reads `exposeMethods` -- a one-letter difference that silently disables the only event telling a page its challenge has been answered, so both spellings are now passed. And `ensureWidget` guarded on `window.sendOtp`, which does not exist until their Angular application finishes booting: a second caller arriving in that gap concluded initialisation had not happened and did it again, appending a second embedded view into the same container and drawing two hCaptcha boxes stacked on each other. React's development double-invoke made it happen on every load. The general shape is what to keep: a third-party script whose readiness is signalled by a global appearing is a script whose initialisation guard must be a promise, not a poll of that global, and a config key that silently does nothing is indistinguishable from a feature the provider does not have.", "apps/web/src/features/auth/msg91-widget.ts"],
    ["technical debt", "Four diagnoses of one button, and the fault was in none of the places they looked. Resend failed from the day MSG91 went live; three fixes went at the captcha and one at the rate limits before somebody read the provider's own script and found that `retryOtp` refuses a null channel on a Custom widget — and refuses it by *throwing*, rather than through the failure callback it was handed. Two properties combined to hide it, and both are general. A provider method that throws where its siblings call back produces a failure shaped like a crash rather than like a refusal, so it carries none of the provider's wording; and the client turned every such failure into the one sentence an outage produces, so the screen said the same thing whatever had gone wrong. What actually settled it was reading `otp-provider.js` and then asking the live widget what it was configured as — a check that costs nothing, was available on day one, and is now `npm run phone:probe`'s section L, which asserts the client against the dashboard because no typecheck, lint or build can see a disagreement between a repository and somebody's settings page.", "apps/web/src/features/auth/msg91-widget.ts, scripts/phone-limits-probe.mjs"],
    ...
    ["technical debt", "One locale key, two clients, opposite truths. `auth.otp.ledePrefix` says SMS checking is not switched on — false on the web since MSG91 went live, and no longer reached by the app either, which sets `phoneVerificationIsLive` and renders a live sentence instead. The string is shared through `packages/i18n`, so correcting it for the web would have lied to the app and deleting it broke the app's typecheck outright. It is kept for the app and the web now reads its own key. Any copy describing what the product *can currently do*, rather than what it is, needs to be per-client or per-state, because the two clients are not at the same stage and will not be until the template arrives. The sentence that replaced it deserves the same scrutiny: it is hardcoded English inside `confirm-phone.tsx`, so it walks straight past the locale files the app was given on 2026-09-23, and it tells the member a code was sent to their number — which, on the build production is serving, is not true (section B). The key kept for the app is now kept for nothing: the branch that reads it cannot be reached while `phoneVerificationIsLive` is set.", "packages/i18n/src/locales/*, apps/mobile/app/onboarding/confirm-phone.tsx, apps/mobile/src/features/onboarding/phone.ts"],
    ...
    ["technical debt", "**The first diagnosis was wrong, and the way it was wrong is the useful part.** Cold starts on this Worker cost 422–659 ms of CPU against 48–99 ms warm; that is measured, reproducible, and was written up here as the probable cause of the 1102. It is not the cause. Cloudflare's own analytics show the killed invocations burning 12.8–65.6 ms of CPU on a usage model whose ceiling is thirty seconds, while healthy requests on the same Worker spend 700–811 ms untouched — so the resource exhausted was read as memory. **SUPERSEDED 2026-09-24 evening -- it is CPU, and the plan is why.** `wrangler tail` reports these invocations as `outcome: exceededCpu` with the exception `Worker exceeded CPU time limit.`, and the kills sit at exactly 10.00 ms. A CPU kill sits *at* the ceiling, and 10 ms is the Workers **free plan** budget -- the plan `wrangler.jsonc` records choosing. The 30 seconds reasoned from below is the *paid* Standard limit, and GraphQL analytics has no `outcome` dimension, which is why it reported `exceededResources` and the memory reading survived a whole day. Startup is not implicated either: the deploy prints `Worker Startup Time: 18-25 ms`. This retro-explains everything -- the locale bundle and the 12-to-1 Supabase client change both cut *per-request* CPU and both helped; removing `@vercel/og` cut *startup* work and changed nothing. A fourth fix (skipping `getUser()` when no `sb-` cookie is present) cut median CPU 34 ms to 23 ms and p90 56 ms to 31 ms, and the signed-in walk now draws 4 `exceededResources` against 87 successes rather than 14. It is not zero and will not reach zero: a dynamic Next.js render costs ~23 ms median against a 10 ms budget, so the remaining options are the paid plan, making public routes static, or leaving Workers. The reasoning preserved below is kept deliberately -- a worked example of a plausible number being adopted as a cause.  What produced the error was a plausible number found while looking for a cause, in a system where the real limit is invisible: Cloudflare kills the invocation, so there is no exception, no stack and no log of Eraya's own, and the only instrument that distinguishes the two ceilings is an API the deploy credentials cannot read. The general shape, worth more than the incident: when a system reports only *that* it failed, the first measurement that looks bad enough will be adopted as the explanation, and nothing in the investigation will contradict it. What settled this was not a better measurement but a falsifiable one — a number the hypothesis could not survive.", "apps/web/wrangler.jsonc"],
    ["launch blocker", "**SUPERSEDED 2026-09-24 evening -- it is CPU, and the plan is why.** `wrangler tail` reports these invocations as `outcome: exceededCpu` with the exception `Worker exceeded CPU time limit.`, and the kills sit at exactly 10.00 ms. A CPU kill sits *at* the ceiling, and 10 ms is the Workers **free plan** budget -- the plan `wrangler.jsonc` records choosing. The 30 seconds reasoned from below is the *paid* Standard limit, and GraphQL analytics has no `outcome` dimension, which is why it reported `exceededResources` and the memory reading survived a whole day. Startup is not implicated either: the deploy prints `Worker Startup Time: 18-25 ms`. This retro-explains everything -- the locale bundle and the 12-to-1 Supabase client change both cut *per-request* CPU and both helped; removing `@vercel/og` cut *startup* work and changed nothing. A fourth fix (skipping `getUser()` when no `sb-` cookie is present) cut median CPU 34 ms to 23 ms and p90 56 ms to 31 ms, and the signed-in walk now draws 4 `exceededResources` against 87 successes rather than 14. It is not zero and will not reach zero: a dynamic Next.js render costs ~23 ms median against a 10 ms budget, so the remaining options are the paid plan, making public routes static, or leaving Workers. The reasoning preserved below is kept deliberately -- a worked example of a plausible number being adopted as a cause. The entry as originally written said: an isolate serving the website exceeded its 128 MB memory ceiling, which is what produced Error 1102 for a member on 2026-09-23. A Worker isolate is not one request's worth of memory: it is a long-lived sandbox serving many requests at once out of a single 128 MB allowance, so what any module holds resident is held on behalf of all of them. Two occurrences in twenty-four hours, and the second is the one that shows the shape — 174 requests at Milan killed in a burst, each having spent ten milliseconds of CPU, which is what it looks like when an isolate is already over the line and every request handed to it dies on arrival rather than one request being expensive. The five that hit a member on 2026-09-23 were the same thing at Paris, and it reached them as a page branded Cloudflare rather than Eraya. Three things follow for a reviewer. It is not attributable to the feature that exposed it: `/login` renders a form for somebody with no session and is as costly to boot as anything else, so finishing onboarding was only the way to arrive at a route an isolate had not served yet. It gets worse rather than better as the product grows, because every route added enlarges what an isolate holds, and nothing in this repository reports that number — types, lints, builds and probes all pass on a Worker that cannot serve a request. And the failure is invisible from the inside: Cloudflare kills the invocation, so there is no exception to catch, no error boundary to render and no row in `auth_events`. The only trace is in Cloudflare's own telemetry; the account's deploy credentials cannot read Workers Logs at all, and the aggregate API that answered it reports a single word, `exceededResources`, for both ceilings. The largest contributor this repository controls — a megabyte of locale dictionaries held resident — has been removed and measured, and is described in the next entry. That is the largest contributor Eraya owns, not the largest contributor: Next's server runtime and React are the bulk of the 128 MB. **Production has now answered, and the answer is no.** With the fix live as version `37501775`, a sequential walk of the signed-in routes at about one request a second produced 14 `exceededResources` at Marseille on 2026-09-24, at 10.0 ms of CPU against 279 ms healthy. The same walk paced three seconds apart passed 9/9. The fix removed a megabyte an isolate was holding and it moved the threshold rather than the ceiling, which is what a reviewer should take from it: this remains open, it is provoked by sustained traffic to signed-in routes rather than by any one page, and the next levers are `/home` reading the session four times per render and the Next runtime itself.", "apps/web/wrangler.jsonc, apps/web/open-next.config.ts"],
    ["technical debt", "**Six languages are parsed to render one.** `packages/i18n/src/index.ts` imports all six dictionaries at module scope, and they compile into a single 507 KB chunk that appears in the server chunks and again in the browser bundle. Every cold isolate therefore evaluates Hindi, Bengali, Marathi, Telugu and Tamil in order to serve an English page, and every visitor downloads all six to read one. The structure is what makes this invisible rather than obvious: a static import of a typed object is exactly what makes the translator synchronous, total and compiler-checked, which are the properties that stopped keys going missing in the first place — so the cost arrived as a consequence of a good decision rather than a careless one. **Changed on 2026-09-23, measured on both sides, and deployed on 2026-09-24 at 06:19:32Z.** The main entry point now holds English alone; the other five are reached through `loadTranslations`, which a bundler splits into a chunk apiece, or through a new `@eraya/i18n/all` for the app, which still wants every language resident and must never await one. The dictionaries the server bundle holds without being asked fall from **1,014 KB to 85 KB** — the two 507 KB chunks become 43 KB and 42 KB, and the five languages become ten on-demand chunks of 86–112 KB, read only when somebody is reading that language. Seven files, none of them in onboarding or the OTP paths, and the app's behaviour is unchanged by design: a phone still downloads all six and still switches without waiting. Verified in `workerd` rather than argued: with each of the six set as the saved preference, the page returns with the right `lang` attribute and twenty-three to thirty-three thousand characters of the right script. What could not be verified locally is the thing that matters — `workerd` does not model isolate memory pressure, and cold-start wall times were 0.37–0.70 s on both builds, which is noise on a laptop. Worth stating as a shape: a workspace package shared by a browser bundle, a Worker and a React Native app is evaluated under three sets of constraints, and the only one with a hard ceiling is the one nothing in the repository measures. The static import that caused this is also what makes the translator synchronous, total and compiler-checked, which is why it survived review for weeks and why the fix keeps those properties on the two clients that can afford them.", "packages/i18n/src/index.ts, packages/i18n/src/all.ts, apps/web/src/features/i18n/server.ts"],
    ["technical debt", "The signed-in home reads the session four times to render once — `loadAuthSession` in the layout, again in the page, and `getUser()` again inside both `getConnections` and `getProfileCompleteness` — on top of the middleware's own call. Each is a round trip to Supabase that validates the same token and returns the same user, and they are the bulk of the 2.4–3.9 second wall times measured against production on 2026-09-23. None of it is wrong in isolation, which is how it accumulated: every one of those functions is correct to establish who is asking rather than to trust a caller. React's `cache()` is the fix and is the smallest one available, deduplicating the read for the lifetime of a request without changing a signature or a guarantee.", "apps/web/src/features/auth/load-session.ts, apps/web/src/features/members/data.ts"],
