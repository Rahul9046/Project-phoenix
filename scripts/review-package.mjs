...
const narrative = {
  ...
  mockedOrIncomplete: [
    ...
    ["Web phone verification reported every failure as an outage. `describeAuthError` renders by error *kind*, and every phone refusal except a cooldown carries the kind `generic`, so a number already registered to another account, a daily cap and a dead widget all read as \"Something went wrong on our side. Please try again.\" A cooldown was worse than generic: it mapped to `rate_limited`, whose copy is about email links and spam folders. Found 2026-09-22 when a second account could not verify a number the first had taken. Fixed in the working tree — the screens now render the sentence written for the status — but `phone-widget-begin` must be deployed before the server stops masking `number_taken`, and it has not been. The halves verify separately and only one of them can be verified at all today: the client half runs locally against the deployed functions, where a taken number correctly produces the shorter \"We could not send your code just now\" — proof the renderer fix works, and the most a local test can establish. The sentence naming the cause cannot be seen until the function ships, so that requirement stands unverified rather than passing", "apps/web/src/features/auth/describeAuthError.ts, phone-verification.ts, supabase/functions/phone-widget-begin", "fixed, awaiting function deploy"],
