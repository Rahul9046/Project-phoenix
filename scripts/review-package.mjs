...
const narrative = {
  ...
  mockedOrIncomplete: [
    ...
    ["The web code screen showed copy from the mocked era: \"Checking codes by SMS is not switched on yet, so any six digits will do for now.\" It was shown to every member on eraya.app while MSG91 was sending real codes and real codes were being required — the screen told people the opposite of what the system was doing. No flag or environment guard was involved; the string was simply never revisited when verification went live. The same screen had no way to ask for another code, so anybody whose SMS did not arrive could only go back. Both are fixed in the working tree, neither is deployed: the screen now names the masked number and carries a resend control on a sixty-second countdown, corrected by the server's own `retryAfter` when it disagrees", "apps/web/src/features/auth/screens/OTPScreen.tsx, packages/i18n/src/locales/*", "fixed, not deployed"],
  ...
  journeys: [
    ...
    ["Phone verification", "Web: MSG91's OTP widget runs in the browser and returns an access token, which `phone-widget-verify` exchanges with MSG91 server-side; the number comes from MSG91's answer and must match the request this account opened. App: the OTP API through `phone-otp-request` / `phone-otp-verify`, because MSG91's widget is a browser SDK and its native SDKs need a config plugin that does not exist.", "**Web: verified in production** on eraya.app with a real Indian number, MSG91 CAPTCHA Validation on and hCaptcha as the provider. **App: not run end to end** — it needs a DLT-approved SMS template first. No code is ever stored, and the fixed development code is refused unless SUPABASE_URL is a local stack. Until 2026-09-22 the web reported every refusal as an outage, including a number already verified on another account — fixed and shipped that day. The code screen names the number it sent to, masked to the last two digits, and offers a resend on a sixty-second countdown that is a label on `begin_phone_otp`'s cooldown rather than a limit of its own — see section B."],
