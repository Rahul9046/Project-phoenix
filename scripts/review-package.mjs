...
const narrative = {
  ...
  mockedOrIncomplete: [
    ["Phone verification works on the web and is proven in production: a real Indian number was verified on eraya.app through the MSG91 widget with hCaptcha enabled. The app is a separate path — it uses the OTP API, and until 2026-09-23 the build production served never called MSG91 at all; see the next row. With that closed, the app now fails honestly and the remaining blocker is a template, which on the same day turned out to be a larger obstacle than \"pending approval\" suggested: Eraya owns no DLT registration at all. `getWidgetProcess` reports the live widget's MOBILE and RETRY processes with an **empty** `templateId`, so the web's SMS ride MSG91's own DLT entity — which is how a working widget and an empty Templates page are both true, and why there is nothing for the app to borrow. The OTP API requires a template registered to Eraya's own Principal Entity, so the app's phone step is blocked on operator paperwork rather than on a dashboard field. As of 2026-09-22 it is **optional on both clients**: a member may verify at onboarding, decline and finish anyway, or verify later from Account → Verification, and declining sends nothing to MSG91 and writes no verification state of any kind", "supabase/functions/phone-otp-* (app), phone-widget-* (web, done)", "app only"],
